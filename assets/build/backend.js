...
(function ($, config, i18n) {
  ...
  const Modal = {
    ...
    show(message, type = 'success') {
      ...
      $(SEL.MODAL_OVERLAY).addClass(CSS.ACTIVE).stop(true, true).fadeIn(200);
    },
    /**
     * Close the modal dialog.
     */
    close() {
      $(SEL.MODAL_OVERLAY).removeClass(CSS.ACTIVE).stop(true, true).fadeOut(200);
    },
    /**
  ...
  const Forms = {
    ...
    init() {
      ...
      form.addEventListener('submit', function (event) {
        ...
        const isDelete = action && 'delete' === action.value || action2 && 'delete' === action2.value;
        if (isDelete && !window.confirm(i18n.__('Delete the selected log entries? This cannot be undone.', 'entire-user-sync'))) {
          event.preventDefault();
        }
      });
    }
